...
def save_sheets():
	with open('data/sheets/color_palette.txt', 'w') as colorFile:
		for i in range(len(colorPalette)):
			pixel = colorPalette[i]
			colorFile.write(f"{pixel[0]} {pixel[1]} {pixel[2]}\n")

	with open('data/sheets/sprite_sheet.txt', 'w') as sheetFile:
